...
for line in lines:
    spl = line.split(',')
    first_spl = spl[0].split('-')
    first_min = int(first_spl[0])
    first_max = int(first_spl[1])
    second_spl = spl[1].split('-')
    second_min = int(second_spl[0])
    second_max = int(second_spl[1])

    if second_min <= first_min <= second_max or second_min <= first_max <= second_max or first_min <= second_min <= first_max or first_min <= second_max <= first_max:
        res += 1
