...
# your_choices = {'X': {'type': 'rock', 'score': 1}, 'Y': {'type': 'paper', 'score': 2}, 'Z': {'type': 'scissors', 'score': 3}}
your_choices = {'X': 'lose', 'Y': 'draw', 'Z': 'win'}
winning_pairs = {'rock': 'paper', 'paper': 'scissors', 'scissors': 'rock'}
losing_pairs = {'rock': 'scissors', 'scissors': 'paper', 'paper': 'rock'}
...
sum = 0
for line in lines:
    opp = opponent_choices[line[0]]
    you = your_choices[line[2]]
    # sum += you['score']
    # if opp == you['type']:
    #     # Draw
    #     sum += 3
    # elif (opp == 'rock' and you['type'] == 'paper') or (opp == 'paper' and you['type'] == 'scissors') or (opp == 'scissors' and you['type'] == 'rock'):
    #     # Win
    #     sum += 6
    if you == 'lose':
        sum += scores[losing_pairs[opp]]
    elif you == 'draw':
        sum += 3
        sum += scores[opp]
    elif you == 'win':
        sum += 6
        sum += scores[winning_pairs[opp]]
